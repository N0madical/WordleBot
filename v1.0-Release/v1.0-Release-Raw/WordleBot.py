...
# Read text file
with open('core/fivewords.txt') as words:
...
def reload():
    ...
    for p in range(0, frames):
        for i in range(0, len(clonehandlerlist)):
            ...
            clonehandlerlist[i].config(bg="#%s" % (rgb_to_hex((rrgb, grgb, brgb))), activebackground="#%s" % (rgb_to_hex((rrgb, grgb, brgb))))
        master.update()
        time.sleep(0.01)
    ...
    frames = 50
    for p in range(0, frames):
        instructions.config(fg="#%s" % (rgb_to_hex((rrgb, grgb, brgb))))
        master.update()
        time.sleep(0.01)
...
def newwordani(prev, char):
    ...
    if (entry == 1):
        ...
        frames = 20
        for p in range(0, frames):
            finishbutton.config(bg="#%s" % (rgb_to_hex((rrgb, grgb, brgb))), activebackground="#%s" % (rgb_to_hex((rrgb, grgb, brgb))))
            restartbutton.config(bg="#%s" % (rgb_to_hex((rrgb, grgb, brgb))), activebackground="#%s" % (rgb_to_hex((rrgb, grgb, brgb))))
            master.update()
            time.sleep(0.01)
